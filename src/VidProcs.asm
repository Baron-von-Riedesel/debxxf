...
@@:
	...
	cmp bh,01				; MDA?
	jz setvideoaddr1
	cmp bh,05				; ega with what???
	jz setvideoaddr1
	cmp bh,07				; vga with monochrom monitor?
	jz setvideoaddr1
	cmp bh,0bh				; model 30 mit monochrom
	jz setvideoaddr1
	mov dl,0D4h
setvideoaddr1:
...
;*** zeichen direkt in videosegment ausgeben
;*** mit Scroll, auch fr MDA/Hercules
...
sm1:
	cmp edi, offset altcrt
	jnz @F
	mov @flat:[__alt_curpos],ax
	mov word ptr @flat:[__alt_curpos+2],?ALTISVALID
	jmp sm2
@@:
...
VioGetCurPosDir proc stdcall public uses edi pCrt:ptr CRTPARMS
	...
	jnz getstdcrtpos
	xor eax,eax
	cmp word ptr @flat:[__alt_curpos+2],?ALTISVALID
	jnz sm2
	mov ax, @flat:[__alt_curpos]
	jmp sm2
getphyspos:
	movzx ecx, byte ptr @flat:[__vio_page]
